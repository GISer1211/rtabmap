...
其中 `μ, σ` 是本次迭代采样像素的均值/标准差。

> **关于雅可比（重要）**：这个归一化残差 `r_p` **就是 GN 真正最小化的量**（进 H/b），不是"验收/护栏指标"。它的**精确**导数还应包含 `∂μ_to/∂ξ`、`∂σ_to/∂ξ`（因为 μ_to、σ_to 通过所有 warp 后的灰度依赖 ξ）。本实现采用标准的**常归一化近似**：只保留逐点项，即图像梯度按 `1/σ_to` 缩放。这并非随意：
> - `∂μ_to/∂ξ` 项对梯度的贡献**恰好为零**——零均值归一化使残差之和为零（`Σr_p=0`）；
> - `∂σ_to/∂ξ` 项**保方向**且在极小值处（残差→0）**消失**，故**不偏移收敛解**。
>
> 已用有限差分对**精确 ZNCC 代价**的梯度做交叉验证：近似解析梯度与有限差分梯度 **cos 夹角 = 0.99999**，且在强仿射光照（增益 2.2、偏置 55）下 GN **收敛到真值**（ATE ≈ 2e-5 m）。即近似雅可比本质精确、解无偏。
...
ZNCC 雅可比的**有限差分交叉验证**：ZNCC 归一化残差是 GN 真正最小化的量（见 §2.5），其近似雅可比（常归一化）与**精确 ZNCC 代价**的有限差分梯度 **cos 夹角 = 0.99999**，且在上述强光照变化下 GN 收敛到真值（ATE ≈ 2e-5 m）。证明近似雅可比本质精确、解无偏（`∂μ` 项因 `Σr=0` 零贡献、`∂σ` 项在极小值处消失）。
